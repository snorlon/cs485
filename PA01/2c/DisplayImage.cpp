...
int main(int argc, char** argv )
{
    ...
    Mat sobel_gradient_magnitude = image.clone();
    ...
    for(int i=0; i<height; i++)
    {
        for(int j=0; j<width; j++)
        {
            ...
            sobel_gradient_magnitude.at<uchar>(i,j) = std::max(0.0, std::min(sqrt((sobel_x * sobel_x) + (sobel_y * sobel_y)), 255.0));
    ...
    imshow("Display Image", sobel_gradient_magnitude);
